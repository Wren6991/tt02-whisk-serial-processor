...
* A processor that can run real programs, with a useful amount of memory
	* Quantifying "real programs": can run a self-hosted assembler
...
Aspirationally: 6x 16-bit registers, r0..r5 (approx 100 bits). Ideally these will be flops, but it's possible to drop down to latches if necessary, e.g. in groups of 4+1 where the last latch is sacrificial, at the cost of some execution speed. Instructions are three-address-code, because we can afford the encoding space, and we are relatively register-poor so would like to avoid unnecessary clobbering. PC is available as register index 7. The second operand may select index 6 to indicate a 16-bit literal following the instruction. (Of course there is nothing stopping you from executing your literals as instructions on different code paths, for bonus style points.)
...
Another ugly thing I noticed when I started actually writing the logic was that when we have a post-decrement with an immediate operand, we need to read the immedate operand in for the post-decrement, as well as potentially reading it in for the pre-increment. This means restarting our sequential fetch one unit earlier.
...
No arithmetic shift. Signed arithmetic is for weenies. (Can be synthesised as SUB(SRL(AND), SRL) at the cost of a clobber and 3 extra instructions).

We have 16 instructions in the first 4 bits, counting the 4 variants each of LDR/STR. The next 3 bits are used to encode condition codes. There are three flags: N Z C (Negative, Zero, Carry), and condition codes are updated by ADD/SUB/ANDN/XOR/SLL/SRL instructions where the destination is not PC. Codes are:

* 000: `al` (always: implicit if no code)
* 001: reserved, if I don't think of something snappy then it will end up as "never"
* 010: `n` (Negative)
* 011: `nn` (not Negative)
* 100: `c` (Carry)
* 101: `nc` (no Carry)
...
`mov rd, rs: andn rd, rs, #0` (hopefully not that common due to three-address-code)
`ldi rd, #imm: xor rd, rd, rd; xor rd, rd, #imm` (hopefully not that common due to cheap immediate operands)
`j rs: add pc, pc, #imm`
